...
//! 非 Windows 平台返回空列表(菜单枚举为空即自然隐藏)。
//!
//! 从 `src-tauri/src/wsl_distros.rs` 原样移入,只去掉 `#[tauri::command]`。
...
mod tests {
    ...
    /// 非 Windows 上恒为空;Windows 上不崩且条目名非空即可(取决于本机是否装了 WSL)
    #[test]
    fn list_wsl_distros_does_not_panic() {
        let list = list_wsl_distros();
