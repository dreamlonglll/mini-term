...
/// 中文条目总数
pub const ZH_ENTRY_COUNT: usize = 879;
/// 英文条目总数
pub const EN_ENTRY_COUNT: usize = 879;
...
static MOBILE_RELAY_ZH: &[(&str, &str)] = &[
    ...
    ("launchersMoved", "手机能起哪些 agent，由「设置 → Shell」里的 AI 启动器决定。"),
...
static MOBILE_RELAY_EN: &[(&str, &str)] = &[
    ...
    ("launchersMoved", "Which agents your phone can start is decided by the AI launchers under Settings → Shell."),
...
static SETTINGS_ZH: &[(&str, &str)] = &[
    ...
    ("launchers.commandWarning", "这条命令不会被识别为 AI 会话：没有状态徽章与对话镜像，手机上会一直等不到会话出现。命令仍会照常保存。"),
    ("launchers.defaultShell", "使用默认 shell"),
    ("launchers.empty", "还没有启动器：「新建终端」菜单里不会出现 AI 启动器段，手机也无法发起新会话。"),
    ("launchers.intro", "「新建终端」菜单里的 AI 启动器，也是手机能起哪些 agent 的名单。命令与 shell 只保存在桌面端，手机只看得到名称。"),
    ("launchers.namePlaceholder", "名称（如 Claude）"),
    ("launchers.orchestration", "允许编排"),
    ("launchers.orchestrationBadge", "编排"),
    ("launchers.orchestrationHint", "用这条启动器起的会话可以启动并驱动其他 AI 会话（范围限本项目与同分组项目），编排礼仪 Skill 会随会话写进项目目录。只在你确实要开编排者时勾选。"),
...
static SETTINGS_EN: &[(&str, &str)] = &[
    ...
    ("launchers.commandWarning", "This command won't be recognised as an AI session — no status badge or transcript mirror, and your phone will wait forever for the session to appear. It will still be saved."),
    ("launchers.defaultShell", "Use default shell"),
    ("launchers.empty", "No launchers yet: the New Terminal menu has no AI launcher section and your phone cannot start new sessions."),
    ("launchers.intro", "The AI launchers offered in the New Terminal menu — and the list of agents your phone can start. Commands and shells stay on the desktop; the phone only ever sees the name."),
    ...
    ("launchers.orchestrationHint", "Sessions started from this launcher may start and drive other AI sessions (within this project and projects in the same group); the orchestrator skill is written into the project alongside the session. Only tick this when you really want an orchestrator."),
