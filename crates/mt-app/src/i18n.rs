...
const USED_KEYS: &[&str] = &[
    ...
    // ⚠️ 这四条是**动态 key**(以 `(&str, &str)` 传进 `spawn_fs_op` 再 `t()`),
    // 上面注释里那条 grep 抓不到它们 —— 靠这张表兜住,别手滑删了
    ...
    "prompt.confirm",
    "prompt.ok",
    // 全局搜索。`search.searchingFound` / `foundFiles` / `foundMatches` /
    // `shortcutHint` 是**带占位符**的动态串,上面那条 grep 抓得到 key 但抓不到
    // 它们走的是 `tr!` / `t_args`,一并列在这里
    "search.copyFilePath",
    "search.foundFiles",
    "search.foundMatches",
    "search.idleHint",
    ...
    "search.title",
    "search.truncated",
    // 分支树三条:live 行 / 非 live 行的 tooltip 与「无法在本机恢复」提示
    ...
    "usageStats.byShell",
    "usageStats.byTool",
    // 单桶摘要卡。`tr!` 写成多行,上面那条 grep 抓不到 —— 靠这张表兜住
    ...
    // Token 副行四项走 `t("usageStats", key)` 的**动态 key**,grep 抓不到
