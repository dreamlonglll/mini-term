...
use gpui::{
    ...
    Styled, Subscription, Task, TitlebarOptions, Window, WindowBounds, WindowOptions, actions, div,
...
fn main() {
    Application::new().run(|cx: &mut App| {
        ...
        let mut bindings = vec![
            ...
            // Git「更改」区的提交框:Ctrl+Enter / Cmd+Enter 直接提交
            // (`GitChanges.tsx:411-415`)。谓词与项目切换器同一套路 ——
            // 必须与 `Input` **同深度**才压得过组件库自带的回车处理。
            KeyBinding::new(
                "ctrl-enter",
