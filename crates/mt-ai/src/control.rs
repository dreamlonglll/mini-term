...
impl Command {
    ...
    /// 这条命令能不能就地在 HTTP 线程上答完。
    ///
    /// [`try_handle_control`] 据此决定要不要另起一条线程 —— hook 那个 HTTP 循环
    /// 是**单线程**的，占着它慢慢干会把排在后面的 hook 上报一起卡住，而 AI 状态
    /// 感知是本仓的权威通道，不给编排让路。
    ///
    /// **占着不放的理由有两种，这张表两种都收**（工单 07 之前它叫
    /// `blocks_on_desktop`，只说得出第一种）：
    ///
    /// 1. **要回桌面主线程**：`start-session` 建 pane、`send` 写 PTY、`read-screen`
    ///    读 gpui 实体手上的终端 —— 都得等主线程回话，最坏是一整个
    ///    [`ACTION_TIMEOUT`]。
    /// 2. **要做慢 IO**：`read-transcript` 要把整条会话记录读进来解析一遍。跑了
    ...
    fn needs_own_thread(self) -> bool {
        match self {
            ...
            Self::StartSession | Self::Send | Self::ReadScreen | Self::ReadTranscript => true,
        }
    }
}
...
mod tests {
    ...
    ///
    /// 两种「占着」都要登记（见 [`Command::needs_own_thread`]）：要回主线程的
    /// （`start-session` 建 pane、`send` 写 PTY、`read-screen` 读 gpui 实体手上的
    /// 终端），以及要做慢 IO 的（`read-transcript` 读整条会话记录）。漏登记任何
    /// 一条都会把 hook 上报那条队一起卡住 —— 正是这张表要防的事。
    #[test]
    fn 占着_http_线程的命令都登记在表里() {
        ...
        assert_eq!(
            own_thread,
            vec!["start-session", "send", "read-transcript", "read-screen"]
        );
    }
