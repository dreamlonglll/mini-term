...
手机上的轻客户端，经中转服务器与桌面端通信。只收发消息：查看活跃 AI 会话的结构与状态、查看对话镜像、发出移动端指令。永远不直接接触 AI 进程或 PTY——所有真实交互都由桌面端代为执行。可见范围仅限处于 AI 会话中的 pane：裸 shell pane 对移动端不可见、不可驱动，移动端也不能启动新的 AI 会话。注意与"远程"（SSH 语境，指远程机器）区分：手机侧概念一律用"移动端"。

## 中转服务器（Relay Server）

部署在公网、在桌面端与移动端之间转发消息的服务。桌面端主动出站连接它（穿 NAT），移动端也连接它。它本身不与 AI、PTY 或项目数据交互。
