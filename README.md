...
Claude / Codex / Grok 之外，终端里跑 **opencode / pi** 也会被认出来——不靠 hook，而是识别你敲下的命令，状态灯、完成播报与手机端发指令照常可用；只是这两家没有可解析的本地会话记录，所以对话镜像、AI 历史面板和用量统计对它们是空的，也不会去蹭同项目里 Claude / Codex 的会话。
