...
| **外置主题包** | 兼容 Dream Skin 格式的皮肤：文件夹或 zip 导入、manifest 的 sha256 校验、改文件即热重载；皮肤可自带背景图，终端随之透明化压在氛围层上。外链一律走同一道闸（禁 `@import`，指向包外的引用全拒）。点「生成示例」落一份可直接改的示例皮肤到皮肤目录；[`theme/`](theme/) 下另备有下载即装的成品皮肤 |
