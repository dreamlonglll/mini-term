...
import type { ShellConfig, EditorConfig, CcConnectStatus, CcConnectConfig } from '../types';

interface Props {
  open: boolean;
  onClose: () => void;
  /** 打开时定位到指定页(便于"顶部状态点 → 设置 → cc-connect"等深链入口)。 */
  initialPage?: SettingsPage;
}

export type SettingsPage = 'terminal' | 'system' | 'font' | 'ai-notification' | 'cc-connect' | 'shortcuts' | 'about';
...
// ─── CcConnectSettings（cc-connect 集成页）───
...
function CcConnectSettings() {
  ...
  // 否则用户每键入一个字符都会触发一次 HTTP probe(尤其在输入半成品路径时会
  // 产生短暂的 "running:false / diagnostic" 状态闪烁)
  ...
  const handleStart = useCallback(async () => {
    ...
    setResultMsg(null);
    try {
      const trimmedExe = exePath.trim();
      if (!trimmedExe) {
        setResultMsg({ kind: 'err', text: '请先填写 cc-connect 可执行文件路径' });
        return;
      }
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      if (status?.running) {
        ...
        setResultMsg({
          kind: 'err',
          text: status?.diagnostic ?? '无法连接到 cc-connect',
        });
      }
    } finally {
      setBusy(null);
  ...
  const handleOpenConfigToml = useCallback(async () => {
    ...
    if (!trimmed) {
      setResultMsg({
        kind: 'err',
        text: '请先用上方"浏览"按钮选择 config.toml 的具体路径(打开操作需要绝对路径)',
      });
      return;
    }
    try {
      await invoke('open_path_with_default_app', { path: trimmed });
    } catch (e: unknown) {
      setResultMsg({ kind: 'err', text: e instanceof Error ? e.message : String(e) });
    }
  }, [configPath]);
  ...
  return (
    <div className="space-y-6">
      {/* 状态指示器 */}
      <div className="px-3 py-3 rounded-[var(--radius-md)] bg-[var(--bg-base)] border border-[var(--border-subtle)]">
        <div className="flex items-center gap-2 mb-1">
      ...
      <div className="pt-1 text-sm text-[var(--text-muted)]">
        重启会断开所有 IM active sessions(chat 历史保留)· mini-term 关闭时不会联动停止 cc-connect
      </div>
    </div>
  );
}
...
const MENU_ITEMS: { key: SettingsPage; label: string }[] = [
  ...
  { key: 'cc-connect', label: 'cc-connect' },
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4">
            ...
            {activePage === 'cc-connect' && <CcConnectSettings />}
