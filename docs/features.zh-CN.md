...
- **oh-my-pi（omp）的 hook 与移动镜像接入** — `omp`（can1357/oh-my-pi，pi 的分支）没有「shell 命令 hook」，它的扩展点是 Bun 运行时**进程内加载的 TS 模块**。注册会把自包含扩展 `miniterm.ts` 写进 `~/.omp/agent/extensions/`（尊重 `PI_CODING_AGENT_DIR` / `PI_CONFIG_DIR` / `PI_PROFILE`），扩展在 omp 进程内直接 `fetch` 本地 hook 服务器。生命周期事件映射到现有 Hook 状态链路；移动端对话镜像则按 hook 上报的 session id 精确定位 `{omp_agent_dir}/sessions/{编码 cwd}/{timestamp}_{session-id}.jsonl`，解析 user / assistant 文本、`ask` toolCall 与 toolResult 作答。手机点选单选题时按 OMP 的 `ask.recommended` 初始高亮合成方向键；自由输入项可点，多选题仍只展示。
- **尚未接会话记录的 agent** — `opencode` / `pi` 没有可解析的本地会话记录：状态徽章、完成播报、AI 启动器与移动端发起会话照常可用，但对话镜像、AI 历史面板与用量统计为空。镜像的启发式绑定由 `agent_has_session_log` 白名单约束，未知 agent 直接返回空镜像，不会绑定同项目里其它 agent 的记录。
