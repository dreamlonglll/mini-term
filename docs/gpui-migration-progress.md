...
- **U 批（dccbc4f 后）记档**：mt-relay +1 行纯 re-export（`StartSessionFailReason`，主会话确认接受——不加则宿主只能恒传 SpawnFailed 丢失败原因档位）；`write_pty` 回执语义弱化为「已排队写入」（预检到落地之间 pane 被关会静默丢，真出投诉再换 oneshot+2s 超时的路 B）；`RelayHost` 镜像有 ≤150ms 陈旧窗口（启动器保存后已额外立即刷一次）；退出时不停 mt-relay 自持 tokio 运行时（Runtime::drop 主线程收尾，理论多几十毫秒，要收紧得给 manager 加 shutdown()）；~~can_start_session 对 SSH 远程项目误判~~（BB-a 已结账：接上「添加远程项目」入口自动生效，to_relay_project 抽出并有测试钉死）；面板正文高度定值 540px（gpui 无视口单位）；面板关闭无钩子（open_guarded 的 on_close 会覆盖 build 里同名回调），配对码到达走 overlay::contains + WeakEntity 双保险。
