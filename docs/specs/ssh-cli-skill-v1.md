...
> 状态：draft · 2026-07-31（rev2：CLI 改为守护进程架构，保留连接池复用）
...
| # | 问题 | v1 决策 |
|---|------|---------|
| 1 | daemon 引入生命周期复杂度（自拉起竞态、孤儿、版本换代、dev 下二进制文件锁定） | 接受，换取池复用；空闲 10min 自退 + 版本握手 + `daemon-stop` 三道兜底 |
| 2 | Codex workspace-write 沙箱默认禁网，CLI 需网络（连 pipe 是否被沙箱拦也未知）→ 可能每次审批 | v1 不写 `network_access = true`（Windows 主平台沙箱为实验状态、影响有限）；实测被拦再评估是否随 enable 写入项目 `.codex/config.toml` |
| 3 | `allowed-tools: 'Bash("<带空格路径>" *)'` 语法未实测 | 实测失败则去掉该行，agent 首次调用手动批准一次 |
| 4 | Codex 对未知 frontmatter 字段（如误留 allowed-tools）容忍度未知 | 两份模板独立生成规避 |
| 5 | skill 触发率不如 MCP 工具列表强曝光 | description 覆盖典型场景；实测触发不稳再在 CLAUDE.md 模板补一句指引 |
| 6 | 远程命令自身 exit 124 与超时歧义 | 接受，stderr 前缀可辅助区分 |
| 7 | daemon 与残存 MCP sidecar 并存时各持独立池（同一连接可能两条 session） | 接受（服务器视角只是两个登录），PR3 下线 MCP 后消失 |

### 10. 分阶段落地

- **PR1**：service 层抽取（`mt-sidecars` lib + `ssh_service` 回调化）+ MCP 适配层瘦身回归。纯重构，行为零变化，`cargo test` 全绿即可合；
- **PR2**：`mt-ssh-cli` 二进制（CLI + daemon + IPC 协议 + 降级路径）+ 构建管线。纯新增，不动注册链路；
- **PR3**：`ssh_skill_registry` + 前端切换 + i18n + MCP 迁移清理 + docs（CLAUDE.md command 表、features 双语）；
- **PR4**（后续版本）：确认存量迁移完成后，移除 `mt-ssh-mcp` bin、`ssh_mcp_registry` 写入路径与 externalBin 条目。
