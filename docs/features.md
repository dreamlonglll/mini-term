...
- **oh-my-pi (OMP) hook and mobile mirror integration** — OMP has no shell-command hooks, so Mini-Term installs an in-process Bun extension at `~/.omp/agent/extensions/miniterm.ts` (honoring `PI_CODING_AGENT_DIR`, `PI_CONFIG_DIR`, and `PI_PROFILE`). Lifecycle events feed the existing hook status pipeline. The mobile conversation mirror uses the hook session id to locate `{omp_agent_dir}/sessions/{encoded-cwd}/{timestamp}_{session-id}.jsonl`, parsing user/assistant text, `ask` tool calls, and tool results. Mobile single-choice answers synthesize keys from OMP's `ask.recommended` initial cursor; free-input entries are actionable, while multi-select remains display-only.
- **Agents without supported session logs** — `opencode` / `pi` still have no parseable local transcript. Status badges, completion announcements, launchers, and mobile-started sessions work, but conversation mirror, history, and usage stay empty. `agent_has_session_log` gates heuristic binding so unknown agents never inherit another agent's newest transcript from the same project.
...
| Tests | **1,737 Rust tests** (27 test targets) + relay-server protocol boundary tests |
...
```bash
# Workspace-wide Rust tests (27 test targets, 1,737 cases)
