...
9. **投递方式已被工单 14 推翻**（2026-09-02）：本票留档里凡是关于「投递闸 / 写穿编排者 /
   批头 / `[mini-term]` 前缀 / 游标要等写穿成功才回写」的条目都只剩考古价值——现在汇报
   落成文件、`wait` 取件。账本、渲染、`transcript_binding`、常驻线程与两处桌面接线
   原样保留，现行形态见 `14-reports-to-files-and-wait.md`。上面第 1、2、3 条在新形态下
   仍然成立（14 的留档里重述了）。
