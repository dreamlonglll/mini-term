...
6. 手机首页右下角 **+** → 选项目 → 选 AI 启动器 → 桌面端在该项目新开一个 tab 并把 agent 拉起来，会话起来后手机自动进入它的对话镜像。启动器在桌面端「移动端」面板里维护（默认预置 Claude / Codex），命令与 shell 只保存在桌面端。
...
3. 桌面端升级到对应版本，并在「移动端」面板填入同一个密钥值，保存并连接。

升级顺序不敏感（旧桌面端连新中转会收到版本不匹配提示，反之亦然），但中途连不上属正常现象，三步做完即恢复。手机侧无需操作：PWA 由中转托管，会自动跟随。
